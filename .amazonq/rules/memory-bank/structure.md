# ASDQ Chart — Project Structure

## Directory Layout
```
ASDQ Chart/
├── main.js                    # Electron main process — app lifecycle, IPC, windows
├── index.html                 # Main window renderer
...
Blank page. Ready for app-specific UI.
...
- `save(settings)` writes to `~/.asdq-chart-settings.json`
...
All user preferences stored in `~/.asdq-chart-settings.json`, read/written by main process via `fs` module.
